...
def hybrid_search(user_id: str, query: str, final_top_k: int = 4, temporary_top_k: int = 100, debug: bool = False) -> list[dict]:
    """ハイブリッド検索を行う関数
    ...
    Returns:
        list[dict]: 検索結果のリスト
    """
    semantic_results = cosine_similar_diary(user_id, query, top_k=temporary_top_k, debug=debug)
